...
def create_build(build: schemas.build.BuildCreate, db: Session = a_db):
    db_build = crud.get_build_by_info(
        db,
        build_name=build.name,
        build_number=build.number
    )
    if db_build:
        raise DataDuplicatedError(
            f'{build.name}의 {build.number}번 빌드는 이미 존재합니다.'
        )
